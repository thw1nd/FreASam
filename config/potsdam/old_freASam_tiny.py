...
from geoseg.datasets.two_modality.online_potsdam_dataset import *
from geoseg.models.FreASam_for_2_modality.FreASam_tiny import FreASam
import os
...
UNFREEZE_SCHEDULE = [
    ...
    {"epoch": 0, "blocks": {"type": "all"}, "parts": ["norm"]},
    # {"epoch": 1, "blocks": {"type": "all"}, "parts": ["mlp"]},

    {"epoch": 1, "blocks": {"type": "all"}, "parts": ["mlp","proj"]},
...
# 分组学习率
lr = 5e-3             # 头部
backbone_lr = 3e-4      # encoder 主干
lr_enc_mlp = 5e-4     # encoder 的 MLP/LoRA
...
weights_name = "FreASam-Tiny-Best"
weights_path = f"model_weights/potsdam/two-modality/{weights_name}"
metrics_txt_path = f"model_weights/potsdam/two-modality/text/{weights_name}"
...
train_dataset = PotsdamDataset(
    data_root=r'H:\Datasets\Potsdam_New\Images\original_train',
    mosaic_ratio=0.0,
    transform=train_aug,
)
val_dataset = PotsdamDataset(
    data_root=r'H:\Datasets\Potsdam_New\Images\original_test',
    mosaic_ratio=0.0,
    transform=val_aug
)
